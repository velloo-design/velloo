...
- **The daemon** = canvas (HTML/JS UI) + HTTP MCP + the authoritative in-memory state + watcher, one detached background process per design folder, keyed by the folder's realpath. Binds the port it last ran on (`.design/cache/port.json`, written after every successful bind), else `:7300`, else a port derived from the folder's path in `7310–7399`, else any free port — so a folder keeps its URL across restarts and a canvas tab left open reconnects. Records `{ pid, canvasUrl, mcpUrl, ports }` in `.design/cache/runtime.json`. Persistent — it outlives any session and self-exits after 5 min idle (no canvas tabs and no agents).
...
- **`CanvasPublish`** (`packages/cli/src/daemon/canvas-publish.ts`) backs `/api/publish`. Because publishing takes tens of seconds, the server holds a **`PublishRunner`** (`packages/server/src/publish-run.ts`) — one run per folder, rejecting a concurrent `POST` with 409 — and the canvas polls `GET /api/publish/status` for the step, capture counter, and warnings, then the share URL. The same interface serves the account's existing links (`GET /api/publish/published`, `DELETE /api/publish/published/:slug`), which go straight to the cloud rather than through the run state: nothing is rendered, so there is no long job to poll. Two failures leave the runner as data rather than a sentence — `signInRequired` and `boardLimit`, the plan's cap on simultaneously published boards — because both are the user's to clear and the canvas answers them with a screen instead of red text.
...
The registry ships 54 shadcn families — every `registry:ui` entry upstream publishes for the Radix base (Accordion, Alert, AlertDialog, AspectRatio, Attachment, Avatar, Badge, Breadcrumb, Bubble, Button, ButtonGroup, Calendar, Card, Carousel, Chart, Checkbox, Collapsible, Combobox, ContextMenu, Dialog, Direction, Drawer, DropdownMenu, Empty, Field, HoverCard, Input, InputGroup, Item, Kbd, Label, Marker, Menubar, Message, NativeSelect, NavigationMenu, Pagination, Popover, Progress, RadioGroup, ScrollArea, Select, Separator, Sheet, Skeleton, Slider, Sonner Toaster, Spinner, Switch, Table, Tabs, Textarea, Toggle, ToggleGroup, Tooltip) — which expand to 281 exported components once parts are counted, plus the 11 framework-neutral Velloo helpers from `@velloo/helpers` (`<Box>`, `<Divider>`, `<Gradient>`, `<Heading>`, `<Icon>`, `<Image>`, `<Layer>`, `<Placeholder>`, `<Prose>`, `<SVG>`, `<Text>`) — the helpers every other provider also reuses.

`vendor.ts` pulls those from upstream's registry, and its `ADAPTED` set names the ones that can't be taken verbatim, skipping them on a routine pull so a re-vendor can't silently revert a shim. Overlay components (Dialog, AlertDialog, Sheet, Popover, DropdownMenu, ContextMenu, Menubar, HoverCard, Combobox, Drawer, Select, Tooltip, Sonner) have their Portal swapped for an inline pinned-open `<div>` — see the snapshot's `canvas-portal.tsx`. Form controls (Input, Textarea, Checkbox, Switch, Slider, ToggleGroup) and collapsible containers (Accordion, Collapsible) promote a handler-less controlled prop to its uncontrolled equivalent, since a design ships `checked` / `value` / `open` with nothing to change them. Calendar takes an ISO date string where react-day-picker wants a `Date`, because a design carries JSON; Chart stays on echarts because recharts renders nothing under static SSR.
...
The IDE chrome's copy at `packages/canvas/src/components/ui/` runs the same pipeline through its own `vendor.ts`, against the same style and CLI pin, and a test compares the two package.jsons — before that existed the copies drifted a full style apart and every family a pull added was simply unavailable to the product. It differs from the snapshot's in three ways, all following from it being a real app rather than a design-mode fork: no canvas-safe rewriting, so its overlays keep real Radix portals; an explicit `COMPONENTS` catalog of what the chrome reaches for, with `registryDependencies` resolved transitively; and a component whose npm dependencies aren't installed is *skipped*, not written, so adding Calendar (react-day-picker) or Command (cmdk) to the chrome has to be a deliberate dependency decision rather than a build failure discovered later. Its `ADAPTED` set holds two files — Slider, which moves `aria-label` onto the thumb where Radix puts `role="slider"` and adds a `tone` for a control showing an inherited value, and Sonner, whose upstream Toaster reads `next-themes`. Two others (Select's `size`, DropdownMenu's icon sizing) stopped being divergences at this pull because upstream shipped them.

Both entry stylesheets also have to define the `cn-*` classes the registry's sources *reference* but upstream doesn't *distribute* — they live in the shadcn.com app's own `globals.css`, the same leak that puts `IconPlaceholder` in the source. An undefined one is inert rather than broken (a menu that isn't frosted, a heading in the body face), so a test in each package asserts every `cn-*` a component uses is defined.
