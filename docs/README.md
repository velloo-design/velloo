...
- An **MCP surface** that exposes the design to your AI agent (Claude Code, Cursor, Codex) — ~55 tools, one per structural operation. Includes structural discovery + mutation, snippet management, theme synthesis (palette derivation, vibe matching, image extraction, contrast scoring), dark-mode audits, screenshots, codegen IR for screens + snippets, theme export, Tailwind class validation, and Claude Haiku-backed SVG generation.
