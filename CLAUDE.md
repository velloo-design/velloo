...
```
schema → result → provider → shadcn-snapshot → provider-none, provider-mui, provider-shadcn-upstream → renderer → codegen → server → canvas → cli
```
