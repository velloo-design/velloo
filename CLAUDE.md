...
- **`@velloo/cli`** — `velloo` binary (citty). Subcommands: `init`, `run`, `design`, `mcp`, `stop`, `status`, `connect`, `login`, `logout`, `publish`, `emit`, `render`, `capture`, `browser`, `export`, `upgrade`, `theme:export`, `completions` (plus the env-gated `trace`, and `folder` as a hidden alias of `design`). Verbs consolidate onto their noun rather than spreading across the top level: `publish --list` / `--remove` replaced `published` / `unpublish`, and `design list|add|remove|move|rename|upgrade|bind|set-app-root` manages the repo's designs (`design add` is `runInit` with the path pre-chosen). The lazy subcommand table lives in `commands/registry.ts` — shared by `cli.ts` and the shell-completions generator (`completions/`), which introspects the citty defs so completions never drift. `init` is interactive (clack-driven wizard); the `wizard/` module composes the prompts and the `scan/` module produces screens from a host app's route structure. **A design's name lives only in its `.design/config.json`.** Resolution (`design.ts`) reads the checkout's designs through `findDesigns` (`packages/server/src/designs.ts`): a repo-root `velloo.json` lists design *paths* inside the repo (plus repo-level preferences such as feedback consent, never per-design config), and `manifest.ts` owns writing it. A design folder outside the checkout (managed storage or a chosen path) is a **local design**, recorded solely in `~/.velloo/designs/.locations/` (`packages/server/src/project-location.ts`). Never write a local design into `velloo.json` or into the checkout (no project agent configs, no guidance files) — a cloned repo must not be able to point Velloo outside itself.
