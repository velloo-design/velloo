...
- **`@velloo/provider-shadcn-upstream`** — The `shadcn-upstream` provider (the default for new folders). Canvas runtime reuses the `@velloo/shadcn-snapshot` registry (avoiding a 25+ `@radix-ui/*` bundle); a real vanilla-shadcn install in the user's app is delegated to `npx shadcn@latest add` at agent handoff. Reads a per-cache `manifest.json` when present (`readManifest`), else the snapshot manifest. (The former in-repo fetch/lockfile/ts-morph-manifest pipeline was pruned as dead — see milestone m-22; recover from git history if a native fetch/install path is ever wired.)
...
- **`@velloo/cli`** — `velloo` binary (citty). Subcommands: `init`, `run`, `mcp`, `stop`, `status`, `connect`, `login`, `logout`, `publish`, `ci`, `emit`, `render`, `theme:export`, `completions` (plus the env-gated `trace`). The lazy subcommand table lives in `commands/registry.ts` — shared by `cli.ts` and the shell-completions generator (`completions/`), which introspects the citty defs so completions never drift. `init` is interactive (clack-driven wizard); the `wizard/` module composes the prompts and the `scan/` module produces screens from a host app's route structure. Folder resolution (`folder.ts`) consults a repo-root `velloo.json` manifest (`manifest.ts`) when present — a monorepo's design folders as named projects, registered by `init`; a pure pointer file, never design config.
