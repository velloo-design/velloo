 ...
 * for inline SVG markup; `generate_image` returns either a Picsum URL
 * seeded by the prompt, or — if `FAL_KEY` is set and `filename` is
 * given — a real raster generated by fal.ai's flux-schnell.
 *
...
export function registerGenerateTools(mcp: McpServer, ctx: MutationContext): void {
  ...
  mcp.registerTool(
    ...
    {
      description:
        "Generate a raster image. Default route: Claude suggests alt text, the URL is a Picsum placeholder seeded by the prompt hash (same prompt → same image, swap for real photo later). If `FAL_KEY` is set and `filename` is given, the tool calls fal.ai flux-schnell and writes the binary to `assets/`. Returns shape for `<Image src=…>`.",
      inputSchema: {
        prompt: z.string().min(1),
        filename: z
          .string()
          .optional()
          .describe(
            "Stem for `assets/<filename>.png`. Required to use fal.ai when FAL_KEY is set.",
          ),
        aspect: z
    ...
    async ({ prompt, filename, aspect, width }) => {
      const r = await generateImage(ctx.folder, prompt, { filename, aspect, width });
      if (!r.ok) return fail(r.error);
      const helperNode = {
        $ref: "Image",
        props: {
