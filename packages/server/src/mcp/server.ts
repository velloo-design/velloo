...
const INSTRUCTIONS = [
  ...
  '  - `type: "node"` params for slot composition — the caller passes a full subtree (e.g. an accent overlay or icon block) — one node, or an array of nodes that render as siblings in the slot (so `actions: [btnA, btnB]` is fine). This is cleaner than threading complex layout decisions through a string param. **For an icon that varies per instance** (driven by status, priority, etc.), use a `node` param, NOT an `icon` param: an `icon` param resolves to one fixed lucide component, so `emit_code` bakes that single icon into the JSX (a lucide name has to be a literal JSX tag) and every instance renders the same glyph — a `node` param emits as a `{slot}` the caller fills per instance. Reserve `icon` params for a single icon chosen once at design time.',
