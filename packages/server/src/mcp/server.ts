...
const INSTRUCTION_PARTS = [
  ...
  "",
  '**Native styling — `set_style`.** A folder targets a framework, and `set_style` routes your style payload to that framework\'s native channel: a Tailwind `className` string for shadcn (where it\'s equivalent to setting `className` directly), an `sx` object for MUI, a plain `style` object for no-framework. So on a shadcn folder `set_style { style: "flex items-center gap-4 p-6" }`; on a MUI folder `set_style { style: { display: "flex", alignItems: "center", gap: 2, p: 3 } }`. Object channels merge shallowly (an inner `null` drops that key); `style: null` clears it. If the payload\'s shape doesn\'t match the channel (a string on MUI, an object on shadcn), the tool rejects it and names the expected shape — that\'s how you discover the active channel. The token model (palette, spacing, radius, typography from `get_theme`) is shared across frameworks; each adapter projects it natively, so prefer theme tokens over hard-coded values in either channel.',
  "",
