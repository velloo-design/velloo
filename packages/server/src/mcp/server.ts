...
const INSTRUCTIONS = [
  ...
  "  - `children` is a *nodes-only* array — you can't interleave raw strings and nodes. Inline text goes in `props.children` as a string; for text + an element on one line, wrap each text run in its own `Text`/`Badge` sibling node.",
