...
export async function moveNode(ctx: MutationContext, args: MoveNodeArgs): Promise<MoveNodeResult> {
  const { pageId, variantId, fromPath, toParent } = args;

  if (fromPath.length === 0) {
    throw new MutationError({
      code: "INVALID_PATH",
      message: "Cannot move the variant root.",
      path: fromPath,
    });
  }
  if (isAncestor(fromPath, toParent) || JSON.stringify(fromPath) === JSON.stringify(toParent)) {
    throw new MutationError({
      code: "INVALID_PATH",
      message: "Cannot move a node into itself or its descendant.",
      path: fromPath,
    });
  }
  ...
  if (!fromParentInfo)
    throw new MutationError({ code: "INVALID_PATH", message: "no parent", path: fromPath });
  ...
  if (!fromParent?.children || fromParentInfo.index >= fromParent.children.length) {
    throw new MutationError({
      code: "INVALID_PATH",
      message: `No node at path ${JSON.stringify(fromPath)}`,
      path: fromPath,
    });
  }
  ...
  if (!target) {
    throw new MutationError({
      code: "INVALID_PATH",
      message: `No parent at ${JSON.stringify(adjustedToParent)}`,
      path: adjustedToParent,
    });
  }
