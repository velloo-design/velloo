...
> **Status — legacy.** This package is the legacy provider, kept registered for back-compat with existing folders (`library.id: "shadcn-react"`). New folders default to [`@velloo/provider-shadcn-upstream`](../provider-shadcn-upstream/), which fetches vanilla shadcn from upstream at a pinned version and applies canvas-safe behavior through [`@velloo/shadcn-adapter`](../shadcn-adapter/).

The pinned shadcn component snapshot Velloo originally shipped with. **Components are embedded here, not in user design folders.**

Contents:

- `src/components/` — vendored shadcn primitives (button, card, input, …) plus Velloo wrappers (`Placeholder`, `Icon`, `Text`, `Heading`). Overlays use a canvas-safe contract (see `canvas-portal.tsx`): Portal/Content replaced with pinned-open inline `<div>`s so the design surface shows the styled state without escaping the iframe. The overlay components are maintained in lockstep with their twins in [`@velloo/shadcn-adapter`](../shadcn-adapter/) — `packages/shadcn-adapter/src/__tests__/parity.test.ts` fails if either side changes without the other (the copies exist because `installSnapshot()` must ship self-contained files).
